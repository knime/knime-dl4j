...
public abstract class AbstractDLLearnerNodeModel extends AbstractDLNodeModel {
    ...
    protected PortObjectSpec[] configure(final PortObjectSpec[] inSpecs, final List<String> selectedColumns,
        ...
        ConfigurationUtils.validateColumnSelection(tableSpec, selectedColumns.toArray(new String[selectedColumns.size()]));
        ...
        final List<Pair<String,String>> inputs = ConfigurationUtils.createNameTypeListOfSelectedCols(selectedColumns, tableSpec);
    ...
    /**
     * Attempts to transfer the {@link Updater} from one {@link MultiLayerNetwork} to another.
     * This is important if you want to further train a pretrained net as some {@link Updater}s
     * contain a history of gradients from training.
     *
    ...
    /**
     * Attempts to transfer weights from one {@link MultiLayerNetwork} to another. The weights will be
     * transfered layer by layer. Weights will only be transfered between intersecting layers of both
     * networks. Assumes that the order and type of the layers to transfer the weights is the same.
     *
        ...
        for(final org.deeplearning4j.nn.api.Layer layer : to.getLayers()) {
            if(i < oldWeights.size()){
                try {
                    ...
                    messages.add("Successfully transfered weights from layer: "
                            + (i+1) + " (" + from.getLayers()[i].getClass().getName() + ") of old network to "
                            + "layer: " + (i+1) + " (" + layer.getClass().getName() + ") of new network");
                } catch (final Exception e) {
                    messages.add("Could not transfer weights from layer: "
                            + (i+1) + " (" + from.getLayers()[i].getClass().getName() + ") of old network to "
                            + "layer: " + (i+1) + " (" + layer.getClass().getName() + ") of new network");
    ...
    /**
     * Attempt to transfer initialisation of one {@link MultiLayerNetwork} to another meaning
     * transferring the weights and the {@link Updater}. Convenience method which calls
     * {@link AbstractDLLearnerNodeModel#transferWeights(MultiLayerNetwork, MultiLayerNetwork)}
     * and {@link AbstractDLLearnerNodeModel#transferUpdater(MultiLayerNetwork, MultiLayerNetwork)}.
     *
     ...
     * @return a list of messages if the transfer was successful and if not a reason of the problem
     */
    protected List<String> transferFullInitialization(final MultiLayerNetwork from, final MultiLayerNetwork to, final boolean transferUpdater){
        final List<String> messages = new ArrayList<>();
        if(from != null){
     ...
     */
    protected void pretrainOneEpoch(final MultiLayerNetwork mln, final DataSetIterator data, final ExecutionContext exec)
            throws CanceledExecutionException{
        while(data.hasNext()){
            exec.checkCanceled();
     ...
     * @throws CanceledExecutionException
     */
    protected void finetuneOneEpoch(final MultiLayerNetwork mln, final DataSetIterator data, final ExecutionContext exec)
            throws CanceledExecutionException{
        while (data.hasNext()) {
            exec.checkCanceled();
            if(m_learningMonitor.checkStopLearning()) {
     ...
     * @throws CanceledExecutionException
     */
    protected void backpropOneEpoch(final MultiLayerNetwork mln, final DataSetIterator data, final ExecutionContext exec)
            throws CanceledExecutionException{
        final boolean isPretrain = mln.getLayerWiseConfigurations().isPretrain();
        /**
         * Need to set pretrain to false here because mln.fit() on a DataSetIterator
         * performs pretraining and finetuneing if pretrain is set to true.
         * Workaround because can't extract backprop procedure from
         * MultiLayerNetwork.fit(DataSetIterator) implementation as it depends
         * on protected methods.
         */
        if(isPretrain){

...
public enum DL4JGradientNormalization {
    ...
    RenormalizeL2PerLayer(GradientNormalization.RenormalizeL2PerLayer),
         ...
         * the layer.<br>
         * This differs from RenormalizeL2PerLayer in that here, each parameter type (weight, bias etc) is normalized
         * separately.<br>
    ...
    RenormalizeL2PerParamType(GradientNormalization.RenormalizeL2PerParamType),
         ...
         * For example, if threshold = 5, then values in range -5&lt;g&lt;5 are unmodified; values &lt;-5 are set to -5;
         * values &gt;5 are set to 5.<br>
         * This was proposed by Mikolov (2012), <i>Statistical Language Models Based on Neural Networks</i> (thesis),
         * <a href="http://www.fit.vutbr.cz/~imikolov/rnnlm/thesis.pdf">http://www.fit.vutbr.cz/~imikolov/rnnlm/thesis.
         * pdf</a> in the context of learning recurrent neural networks.<br>
         * Threshold for clipping can be set in Layer configuration, using gradientNormalizationThreshold(double
         * threshold)
         * </p>
         */
    ClipElementWiseAbsoluteValue(GradientNormalization.ClipElementWiseAbsoluteValue),
         ...
         * <i>if and only if</i> the L2 norm of the gradients (for entire layer) exceeds a specified threshold.
         * Specifically, if G is gradient vector for the layer, then:
         * <ul style="list-style-type:none">
         * <li>GOut = G &nbsp;&nbsp;&nbsp; if l2Norm(G) < threshold (i.e., no change)</li>
         * <li>GOut = threshold * G / l2Norm(G) &nbsp;&nbsp;&nbsp; otherwise</li>
         * </ul>
         * Thus, the l2 norm of the scaled gradients will not exceed the specified threshold, though may be smaller than
         * it<br>
    ...
    ClipL2PerLayer(GradientNormalization.ClipL2PerLayer),
         ...
         * clipping on each parameter type separately.<br>
         * For example in a recurrent neural network, input weight gradients, recurrent weight gradients and bias
         * gradient are all clipped separately. Thus if one set of gradients are very large, these may be clipped while
         * leaving the other gradients unmodified.<br>
         * Threshold for clipping can be set in Layer configuration, using gradientNormalizationThreshold(double
         * threshold)
         * </p>
         */
